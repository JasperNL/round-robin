   ...
   )
{
   ...
   for( i = 1; i < argc; ++i )
   {
      ...
      else if( strcmp(argv[i], "-x") == 0 )
      {
         ...
         {
            SCIPinfoMessage(scip, NULL, "missing solution filename after parameter '-x'\n");
            paramerror = TRUE;
         }
      }
...
/** Set up the problem-structure and solve SRR */
static
SCIP_RETCODE SCIPrunSRR(
   int                   argc,               /**< number of shell parameters */
   char**                argv,               /**< array with shell parameters */
   const char*           defaultsetname      /**< name of default settings file */
   )
{
   ...
   SCIP_CALL( SCIPincludeSRRPlugins(scip) );
   ...
   )
{
   ...
   retcode = SCIPrunSRR(argc, argv, "scip.set");
